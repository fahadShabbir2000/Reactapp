...
const handleSelectedRow = (row: any) => {
  const schTblBody = document.getElementById('garagestallListTableBody');
...
const GarageStallPage = ({ garageStalls, meta, actions }: GarageStallList) => {
  // useEffect(() => {
  ...
  const handleDelete = (e: any) => {
    ...
    if (confirmModalData.id) {
      setTimeout(() => {
        actions.deleteGarageStall(confirmModalData.id);
        closeConfirmModal();
      }, 1000);
    } else {
      closeConfirmModal();
  ...
  const handleEdit = (e: any, garageStall: GarageStall) => {
    e.preventDefault();
    setFormData({ ...defaultState, ...garageStall });
  };
  ...
  const handleSubmit = (e: any) => {
    ...
    if (formData.name) {
      if (!formData.id) {
        actions.addGarageStall(formData);
      } else {
        actions.updateGarageStall(formData);
      }
    }
  };
    ...
    const initialgaragestallsData: any = [];
    const [gStallsData, setGstallsData] = useState(initialgaragestallsData);

    useEffect(() => {
      // console.log("Total Data Length =>", jobOrdersData.length);
      setTotalItems(garageStallsData.length);

      //Current Page slice
      let newSliceData = JSON.parse(JSON.stringify(garageStallsData)).slice(
          (currentPage - 1) * itemPerPage,
          (currentPage - 1) * itemPerPage + itemPerPage
      );
      setGstallsData(newSliceData);
      console.log(newSliceData);

    }, [
      garageStalls, garageStallsData, currentPage, search, sorting
    ]);
    ...
    const getGarageStallsPaginated = (selectedPage: number) => {
      const limit = itemPerPage;
      setCurrentPage(selectedPage);
      const options = { page: selectedPage, limit };
      actions.getGarageStallList(options);
    };

    useEffect(() => {
      actions.getGarageStallList({ ... paginationOptions })
    }, [actions.getGarageStallList]);

    useEffect(() => {
      getGarageStallsPaginated(currentPage);
    }, [itemPerPage])

    const showData = () => {
      if (garageStalls.loading) {
        return (
          <tr>
      ...
      if (!_.isEmpty(garageStallsData)) {
          return (
            <>
              {garageStallsData.map((garageStall: any, i:any) => (
                <tr key={garageStall.id} id={`tblRow${garageStall.id}`} onClick={()=>handleSelectedRow(garageStall)}>
                  <td>
                    {(((currentPage >= 2) ? ((currentPage-1) * itemPerPage) + (i+1) :  (i+1)))}
                  </td>
                  <td>
                    <strong>{garageStall.name}</strong>
                  </td>
                  <td>
                    {garageStall.status == 1 ? 'Active' : 'In-Active'}
                  </td>
                  <td>
                    <a
                      href="#"
                      ...
                      onClick={(e) => handleEdit(e, garageStall)}
                    >
                    ...
                    <a
                      ...
                      title="Edit"
                      className="text_red"
                      onClick={(e) => handleConfirmModal(e, garageStall.id)}
                    >
      ...
      if (garageStalls.error) {
        return <p>{ garageStalls.error }</p>
      }
              ...
              <div className="row">
                <div className="col-md-3">
                  <h3>
                    Garage Stalls
                    <br />
                    <small>Manage Garage Stall & details</small>
                  </h3>
                </div>
            ...
            <div className="card-body">
              ...
              <div className="table-responsive">
                <table className="table table-bordered table-striped">
                  ...
                  <tbody id='garagestallListTableBody'>
                    {showData()}
                  </tbody>
                </table>
                
                <div className="row" style={{marginLeft: '0px', marginRight: '0px'}}>
                  { !_.isEmpty(garageStallsData) && (
                    <>
                      <div className="col-md-6">
                        <ReactPaginate
                          ...
                          onPageChange={(data) => getGarageStallsPaginated(data.selected + 1)}
                          previousLabel={'<'}
                          nextLabel={'>'}
                          breakLabel={'...'}
...
GarageStallPage.propTypes = {
  // garageStalls: PropTypes.object.isRequired,
  // actions: PropTypes.func.isRequired
};


const mapStateToProps = (state: GarageStallReduxProps) => ({
  garageStalls: state.garageStalls,
  meta: state.garageStalls.meta,
});

const mapDispatchToProps = (dispatch: Dispatch) => ({
  actions: {
    getGarageStallList: bindActionCreators(garageStallActions.getGarageStallList, dispatch),
    getAllGarageStalls: bindActionCreators(garageStallActions.getAllGarageStalls, dispatch),
    getGarageStall: bindActionCreators(garageStallActions.getGarageStall, dispatch),
    addGarageStall: bindActionCreators(garageStallActions.addGarageStall, dispatch),
    updateGarageStall: bindActionCreators(garageStallActions.updateGarageStall, dispatch),
    deleteGarageStall: bindActionCreators(garageStallActions.deleteGarageStall, dispatch),
  },
});
