...
const handleSelectedRow = (row: any) => {
  const schTblBody = document.getElementById('housetypeListTableBody');
...
const HouseTypePage = ({ houseTypes, meta, actions }: HouseTypeList) => {
  // useEffect(() => {
  ...
  const handleDelete = (e: any) => {
    ...
    if (confirmModalData.id) {
      setTimeout(() => {
        actions.deleteHouseType(confirmModalData.id);
        closeConfirmModal();
      }, 1000);
    } else {
      closeConfirmModal();
  ...
  const handleEdit = (e: any, houseType: HouseType) => {
    e.preventDefault();
    setFormData({ ...defaultState, ...houseType });
  };
  ...
  const handleSubmit = (e: any) => {
    ...
    if (formData.name) {
      if (!formData.id) {
        actions.addHouseType(formData);
      } else {
        actions.updateHouseType(formData);
      }
    }
  };
    ...
    const initialhouseTypesData: any = [];
    const [houseNewTypeData, setHouseNewTypeData] = useState(initialhouseTypesData);

    useEffect(() => {
      // console.log("Total Data Length =>", jobOrdersData.length);
      setTotalItems(houseTypesData.length);

      //Current Page slice
      let newSliceData = JSON.parse(JSON.stringify(houseTypesData)).slice(
          (currentPage - 1) * itemPerPage,
          (currentPage - 1) * itemPerPage + itemPerPage
      );
      setHouseNewTypeData(newSliceData);
      console.log(newSliceData);

    }, [
      houseTypes, houseTypesData, currentPage, search, sorting
    ]);
    ...
    const getHouseTypesPaginated = (selectedPage: number) => {
      const limit = itemPerPage;
      setCurrentPage(selectedPage);
      const options = { page: selectedPage, limit };
      actions.getHouseTypeList(options);
    };

    useEffect(() => {
      actions.getHouseTypeList({ ... paginationOptions })
    }, [actions.getHouseTypeList]);

    useEffect(() => {
      getHouseTypesPaginated(currentPage);
    }, [itemPerPage])
    ...
    const showData = () => {
      ...
      if (!_.isEmpty(houseTypesData)) {
          return (
            <>
              {houseTypesData.map((houseType: any, i:any) => (
                <tr key={houseType.id} id={`tblRow${houseType.id}`} onClick={()=>handleSelectedRow(houseType)}>
                  <td>
                  {(((currentPage >= 2) ? ((currentPage-1) * itemPerPage) + (i+1) :  (i+1)))}
                  </td>
                  <td>
                    <strong>{houseType.name}</strong>
                  </td>
                  <td>
                    {houseType.status == 1 ? 'Active' : 'In-Active'}
                  </td>
                  <td>
                    <a
                      href="#"
                      ...
                      onClick={(e) => handleEdit(e, houseType)}
                    >
                    ...
                    <a
                      ...
                      title="Edit"
                      className="text_red"
                      onClick={(e) => handleConfirmModal(e, houseType.id)}
                    >
              ...
              <div className="row">
                <div className="col-md-3">
                  <h3>
                    House Type
                    <br />
                    <small>Manage House Types & details</small>
                  </h3>
                </div>
            ...
            <div className="card-body">
              ...
              <div className="table-responsive">
                <table className="table table-bordered table-striped">
                  ...
                  <tbody id='housetypeListTableBody'>
                    {showData()}
                  </tbody>
                </table>

                <div className="row" style={{marginLeft: '0px', marginRight: '0px'}}>
                  { !_.isEmpty(houseTypesData) && (
                    <>
                      <div className="col-md-6">
                        <ReactPaginate
                          ...
                          onPageChange={(data) => getHouseTypesPaginated(data.selected + 1)}
                          previousLabel={'<'}
                          nextLabel={'>'}
                          breakLabel={'...'}
...
HouseTypePage.propTypes = {
  // houseTypes: PropTypes.object.isRequired,
  // actions: PropTypes.func.isRequired
};


const mapStateToProps = (state: HouseTypeReduxProps) => ({
  houseTypes: state.houseTypes,
  meta: state.houseTypes.meta,
});

const mapDispatchToProps = (dispatch: Dispatch) => ({
  actions: {
    getHouseTypeList: bindActionCreators(houseTypeActions.getHouseTypeList, dispatch),
    getAllHouseTypes: bindActionCreators(houseTypeActions.getAllHouseTypes, dispatch),
    getHouseType: bindActionCreators(houseTypeActions.getHouseType, dispatch),
    addHouseType: bindActionCreators(houseTypeActions.addHouseType, dispatch),
    updateHouseType: bindActionCreators(houseTypeActions.updateHouseType, dispatch),
    deleteHouseType: bindActionCreators(houseTypeActions.deleteHouseType, dispatch),
  },
});
