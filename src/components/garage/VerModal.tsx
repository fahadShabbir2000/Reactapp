...
const VerModal = ({
  ...
  const getSelectedBillingItem = (index: number) => {
    const items = verDefault.filter((item: any) => {
      // console.log('sooooooooooooooooooo', item);
  ...
  const renderHouseLevelTypesHeading = (index: number, value: any) => {
    ...
    if (getHouseLevelTypeItemValue(value, 'garage')) {
      return (
        <label>
          { houseName}
  ...
  const renderBillingItemsInputList = (rowIndex: number, billingItems: any, readOnly: boolean = false, houseLevelTypeId: any = false) => {
    const items = [];
    for (let i = 1; i <= 6; i++) {
      items.push(<>{renderBillingItemInput(rowIndex, i, billingItems, readOnly, houseLevelTypeId)}</>);
    }
    return items;
  };

  const renderBillingItemInput = (rowIndex: number, index: number, billingItems: any, readOnly: boolean, houseLevelTypeId: any = false) => {
    ...
    if (readOnly) {
      ...
      if (getHouseLevelTypeItemValue(houseLevelTypeId, 'garage')) {
        return (
          ...
          <div className="form-group row">
            {verDefault.length > 0 ? verDefault.map((singleLevel: any, i: any) => (
              <>
                ...
                {renderBillingItemsInputList(singleLevel.rowOrder, singleLevel.billingItems, true, singleLevel.houseLevelTypeId)}
